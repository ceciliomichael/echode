...
export class MarkdownViewerManager {
  ...
  getCurrentDocumentPath(): string | undefined {
    return this.context.workspaceState.get<string>('markdownViewer.currentDocumentPath');
  }

  private openDocumentInfo(documentInfo: MarkdownPreviewDocumentInfo): void {
    if (documentInfo.filePath) {
      this.context.workspaceState.update('markdownViewer.currentDocumentPath', documentInfo.filePath);
    }
      ...
      case 'openMarkdownLink':
        if (message.href) {
          ...
          if (docInfo && docInfo.filePath) {
            try {
              const [linkPath] = message.href.split('#');
